...
$cards = $cardObj->getCardsByPage('portfolio');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class ="bg-gray-300">
    ...
    <div class="flex flex-col min-h-screen mb-20">
            ...
            <!--  title -->
            <h1 class="text-2xl text-center font-semibold mt-20">Portfolio</h1>
            <!-- filter buttons -->
            ...
            <div class="flex justify-center gap-2 mb-8">
                ...
                <button onclick="filterCards('web')" class="px-4 py-2 bg bg-gray-600 text-white rounded hover:bg-orange-400 hover:text-black transition ease-in-out duration-700">Web Development</button>
                <button onclick="filterCards('design')" class="px-4 py-2 bg bg-gray-600 text-white rounded hover:bg-yellow-400 hover:text-black transition ease-in-out duration-700">Graphic Design</button>
                <button onclick="filterCards('analysis')" class="px-4 py-2 bg bg-gray-600 text-white rounded hover:bg-green-400 hover:text-black transition ease-in-out duration-700">Data Analysis</button>
                <button onclick="filterCards('others')" class="px-4 py-2 bg bg-gray-600 text-white rounded hover:bg-blue-400 hover:text-black transition ease-in-out duration-700">Others</button>
        ...
        <div class="bg-white border-[10px] border-gray-400 p-6 rounded-lg w-[600px] relative gap-2">
            ...
            <form id="addCardForm" method="post" action="backend/add_cardPort.php">
            <!-- Category -->
      ...
      <!-- Edit Card Modal -->
        <div id="editCardModal" class="fixed hidden inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white p-6 border-[10px] border-gray-400 rounded-lg w-[600px]">
                <h2 class="text-xl font-semibold mb-4">Edit Card</h2>

                <form method="post" action="backend/edit_cardPort.php">
            ...
            <?php foreach ($cards as $card): ?>
                ...
                <div class="card <?= htmlspecialchars($card['category']) ?> max-w-lg p-6 bg-white border border-gray-400 rounded-lg shadow-xl text-black">
                    ...
                    <?php foreach ($links as $ln): ?>
                        <a href="<?= htmlspecialchars($ln['url']) ?>" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center rounded-lg bg-gray-300 hover:bg-green-500 transition ease-in-out duration-7000">
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        ...
                        <!-- Delete -->
                        <form method="post" action="backend/delete_cardPort.php" onsubmit="return confirm('Are you sure you want to delete this card?');">
                            <input type="hidden" name="card_id" value="<?= $card['card_id'] ?>">
                            <button type="submit" class="px-3 py-1 bg-gray-500 text-white rounded text-sm hover:bg-red-500 transition ease-in-out duration-7000">Delete</button>
                        </form>
                    </div>
                </div>
            <?php endforeach; ?>
